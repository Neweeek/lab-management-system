...
public class ApiController {
  ...
  /** 全系统统一时间基准，见 {@link LabTime}。禁止直接使用 {@code LocalDateTime.now()}。 */
  private String now() { return LabTime.nowText(); }
  ...
  /** 容器健康检查专用：不返回任何业务数据，因此可以保持免登录。 */
  @GetMapping("/health") Map<String, String> health() { db.queryForObject("SELECT 1", Integer.class); return Map.of("status", "ok"); }

  /**
   * 工位平面图。需要登录。
   *
   * <p>占用人姓名只对管理员返回：未登录或普通成员请求时 {@code occupant_name} 为 null，
   * 只有 {@code occupant_id} 用于前端判断"这个工位是不是我的"。
   * 修复前此接口免登录且返回所有占用人真实姓名，等于对外公开了全实验室人员名册。
   */
  @GetMapping("/seats") List<Map<String, Object>> seats(HttpSession session) {
    long userId = uid(session);
    boolean admin = "ADMIN".equals(session.getAttribute("role"));
    return db.queryForList("SELECT s.id,s.code,s.row_no,s.col_no,s.area,s.type,s.status,s.occupant_id,s.review_mode,"
        + "CASE WHEN ?=1 OR s.occupant_id=? THEN u.name ELSE NULL END occupant_name "
        + "FROM seats s LEFT JOIN users u ON s.occupant_id=u.id ORDER BY s.row_no,s.col_no", admin ? 1 : 0, userId);
  }
  ...
  Map<String, String> applySeat(@RequestBody Map<String, Object> body, HttpSession session) {
    long userId = uid(session); long seatId = id(body, "seatId");
    // 必须作为事务里的第一条语句抢到 SQLite 写锁，否则"先读后写"的两个并发事务
    // 会在锁升级时立刻拿到 SQLITE_BUSY（busy_timeout 对升级冲突无效）。
    SqliteLocks.acquire(db, "seats", seatId);
    var member = profile(userId);
    ...
    // 抢占式占用：把 status='AVAILABLE' 作为条件，并检查受影响行数。
    // 修复前这里不检查返回值，两个并发请求会同时通过前面的 SELECT 检查，
    // 结果同一工位留下多条 PENDING 申请（甚至被分配给两个人）。
    if (db.update("UPDATE seats SET status='PENDING' WHERE id=? AND status='AVAILABLE'", seatId) == 0) throw new ResponseStatusException(HttpStatus.CONFLICT, "该工位刚刚被其他人申请，请刷新后重新选择");
    try {
      db.update("INSERT INTO seat_applications(user_id,seat_id,reason) VALUES(?,?,?)", userId, seatId, input(body, "reason"));
    } catch (org.springframework.dao.DataAccessException e) {
      // 兜底：V3 建立的两个部分唯一索引。sqlite-jdbc 对部分唯一索引的冲突抛
      // UncategorizedSQLException 而不是 DuplicateKeyException，因此按消息判断。
      if (SqliteLocks.isUniqueViolation(e)) throw new ResponseStatusException(HttpStatus.CONFLICT, "该工位或你本人已有待审核申请，请刷新后查看");
      throw e;
    }
  ...
  Map<String, String> book(@RequestBody Map<String, String> body, HttpSession session) {
    ...
    // 抢占写锁作为事务第一条语句：预约冲突检测是"先读后写"，不加锁会在并发下
    // 抛 SQLITE_BUSY（锁升级冲突），也会让两个请求同时通过冲突检查。
    SqliteLocks.acquire(db, "seats", seatId);
    var me = profile(userId);
    ...
    LocalDateTime start = LocalDateTime.parse(body.get("startAt")); LocalDateTime end = LocalDateTime.parse(body.get("endAt"));
    LocalDateTime current = LabTime.now();
    if (!start.isAfter(current)) throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "预约开始时间必须晚于当前时间");
  ...
  Map<String, Object> availability(@RequestParam(required = false) String from, @RequestParam(required = false) String to, HttpSession session) {
    long userId = uid(session);
    LocalDateTime start = from == null || from.isBlank() ? LabTime.now().withMinute(0).withSecond(0).withNano(0) : LabTime.storedTime(from);
    LocalDateTime end = to == null || to.isBlank() ? start.plusDays(7) : LabTime.storedTime(to);
    ...
    var reservations = db.queryForList("SELECT b.id,b.seat_id,s.code,b.start_at,b.end_at,b.status,CASE WHEN b.user_id=? THEN 1 ELSE 0 END mine,CASE WHEN b.status='APPROVED' AND b.start_at<=? AND b.end_at>? THEN u.name ELSE NULL END current_user_name FROM seat_bookings b JOIN seats s ON s.id=b.seat_id JOIN users u ON u.id=b.user_id WHERE b.status IN ('PENDING','APPROVED') AND b.start_at<? AND b.end_at>? ORDER BY b.start_at", userId, current, current, end.toString(), start.toString());
  ...
  Map<String, String> submitReport(@PathVariable long taskId, @RequestBody Map<String, Object> body, HttpSession session) throws Exception {
    long userId = uid(session); var task = one("SELECT t.*,r.start_at review_start_at FROM weekly_report_tasks t JOIN reviews r ON r.id=t.review_id WHERE t.id=? AND t.member_id=? AND r.status IN ('PLANNED','ACTIVE','AWAITING_DECISION')", taskId, userId); LocalDateTime periodStart = reportPeriodStart(((Number) task.get("review_id")).longValue(), ((Number) task.get("period_index")).intValue(), String.valueOf(task.get("review_start_at"))); LocalDateTime current = LabTime.now(); if (current.isBefore(periodStart)) throw new ResponseStatusException(HttpStatus.FORBIDDEN, "下一周周报尚未开始，当前只能填写当周材料"); String mode = input(body, "mode"); if(!Set.of("save","submit").contains(mode)) throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"周报操作不合法"); String status = "save".equals(mode) ? "DRAFT" : (current.isAfter(LabTime.storedTime(task.get("due_at"))) ? "LATE_SUBMITTED" : "SUBMITTED");
    String sections = json.writeValueAsString(normalizeReportSections(body.getOrDefault("sections", Map.of())));
    // 时间戳一律由 Java 以统一基准写入；不再使用 CURRENT_TIMESTAMP（那是 UTC，会与本地时间列混用）。
    String stamp = LabTime.nowText();
    if (exists("SELECT id FROM weekly_reports WHERE task_id=?", taskId)) db.update("UPDATE weekly_reports SET sections_json=?,status=?,submitted_at=CASE WHEN ?='DRAFT' THEN submitted_at ELSE ? END,updated_at=? WHERE task_id=?", sections, status, status, stamp, stamp, taskId);
    else db.update("INSERT INTO weekly_reports(task_id,user_id,sections_json,status,submitted_at,updated_at) VALUES(?,?,?,?,CASE WHEN ?='DRAFT' THEN NULL ELSE ? END,?)", taskId, userId, sections, status, status, stamp, stamp);
