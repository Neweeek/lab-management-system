...
export default {
  ...
  setup(props:any) {
    ...
    const time=(value:any)=>value?String(value).replace('T',' ').slice(0,16):'—';
    onMounted(()=>search());
    return{rows,total,page,busy,error,selected,filters,labels,sections,search,reset,open,safeUrl,time};
  },
  template:`<section class="management report-archive" :aria-busy="busy">
    ...
    <template v-if="!selected">
      ...
      <nav class="mg-pagination" aria-label="报告分页"><span>第 {{page}} / {{Math.max(1,Math.ceil(total/12))}} 页</span><button class="mg-button" :disabled="busy||page<=1" @click="search(page-1)">上一页</button><button class="mg-button" :disabled="busy||page*12>=total" @click="search(page+1)">下一页</button></nav>
    </template>
